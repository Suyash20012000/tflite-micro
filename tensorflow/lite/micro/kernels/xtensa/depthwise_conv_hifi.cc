...
TfLiteStatus DepthwiseConvInt8EvalHifi(TfLiteContext* context, TfLiteNode* node,
...
#if HIFI_VFPU
TfLiteStatus DepthwiseConvFloat32EvalHifi(TfLiteContext* context, TfLiteNode* node,
                                   const TfLiteDepthwiseConvParams& params,
                                   const XtensaDepthwiseConvOpData& data,
                                   const TfLiteEvalTensor* input,
                                   const TfLiteEvalTensor* filter,
  ...
  if ((params.dilation_width_factor == 1) &&
    ...
    const float output_activation_min =
        DepthwiseConvParamsFloat(params, *(static_cast<const OpDataConv*>(node->user_data))).float_activation_min;
    const float output_activation_max =
        DepthwiseConvParamsFloat(params, *(static_cast<const OpDataConv*>(node->user_data))).float_activation_max;
    TFLITE_DCHECK_LE(output_activation_min, output_activation_max);
